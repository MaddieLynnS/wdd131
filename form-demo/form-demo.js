...
function validateForm(event) {
    ...
    if(theForm.payment.value === "creditCard") {
      if (theForm.userCard.value !== 1234123412341234)
      {
        isValid = false;
    ...
    if(theForm.userName.value !== "Bob Thompson") {
      isValid = false;
  ...
  function togglePaymentDetails(e) {
    ...
    theForm.userCard.required = false;
    theForm.userPaypal.required = false;
  
  
    // Show the container based on the selected payment method, and add the required attribute back.
    if(theForm.payment.value === "creditCard")
    {
      theForm.userCard.required = true;
      creditCardContainer.classList.remove(".hide");
    } else if (theForm.payment.value === "Paypal")
      {
        theForm.userPaypal.required = true;
